# Marmot SDK: Programmatic Agent Orchestration

While Marmot provides a best-in-class CLI and TUI, enterprise developers often need to embed agentic workflows directly into their own applications (e.g., CI/CD pipelines, custom automated code reviewers, or bespoke internal platforms).

To solve this, Marmot exposes the **`marmot-sdk`**, heavily inspired by the best patterns from the Anthropic Claude SDK and the OpenCode SDK.
...
const client = new MarmotClient({ provider: "claude-3-5-sonnet" });
...
```python
from marmot_sdk import MarmotSession, tool
...
Because Marmot is built on an asynchronous Event Bus, the SDK allows you to subscribe to the live telemetry of the agent, enabling you to build your own custom UIs easily.
...
* **Model Agnostic**: Like OpenCode, the `marmot-sdk` is not locked to Anthropic. You can hot-swap to OpenAI, Gemini, or a local Ollama instance via the `marmot-core` Provider trait.
* **True Isolation**: When you create a `MarmotSession` in the SDK, it spins up a unique local socket (the A2A protocol) and an isolated Git Worktree. The SDK client guarantees that the agent cannot corrupt your main branch.
