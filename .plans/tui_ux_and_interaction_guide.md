# Marmot TUI: UX & Interaction Guide

> **Design Philosophy**: The Marmot Terminal UI (TUI) must feel instantaneous, keyboard-centric, and entirely non-blocking. It should provide "progressive disclosure"—keeping the main chat clean while allowing deep dives into tool logs, diffs, and subagent states when needed.

---

## 1. Core Visual Language & Palette

To make the CLI feel like a premium IDE rather than a basic script, Marmot uses a semantic color palette and consistent typography (using Unicode box-drawing characters and Nerd Fonts if available).
...
│ ❯ Marmot is writing `src/db.rs`...    ││ + use std::sync::Arc;                │
│                                       ││                                      │
╰───────────────────────────────────────╯╰──────────────────────────────────────╯
╭─ Prompt ──────────────────────────────────────────────────────────────────────╮
...
| **`Ctrl+D`** | Global | Exit Marmot entirely |
...
To achieve this UX in the Rust backend (`marmot-tui`):
1. **Event Loop**: Use `crossterm` `EventStream` intertwined with the `mpsc::Receiver` from `marmot-server` using `tokio::select!`. This guarantees the UI never locks up while waiting for the LLM.
...
Because Marmot writes every event to the `marmot.db` SQLite store in real-time, sessions are inherently persistent.
* **Auto-Save**: You never need to explicitly "save". If your terminal crashes, the session is already on disk.
* **Resume**: Run `marmot resume` to load your most recent session exactly where you left off.
...
* **Queue**: Because input is non-blocking, if Marmot is busy writing a 500-line file, you can type *"Also make sure to update the README"* and hit Enter. The prompt is **queued** and appended to the agent's context for its next turn.
...
* **Image Rendering**: Using the `Kitty Graphics Protocol` and `Sixel`, Marmot can render image previews directly in the terminal.
* **Multimodal Prompts**: If you drag and drop an image into the terminal, or type `/attach design.png`, Marmot displays a thumbnail of the image inline before sending it to the Vision model.
...
True to the Marmot microkernel philosophy, themes are **not** hardcoded into the Rust binary. Theming and UI layouts are injected via the Plugin System.
* **Theme Plugins**: A Wasm plugin can register a new color palette dynamically by emitting a `RegisterTheme` event to the TUI (e.g., a community `marmot-theme-catppuccin.wasm` plugin).
