# Marmot Project Blueprint

Welcome to the comprehensive design, architecture, and implementation documentation for **Marmot** — a high-performance, configurable, agentic coding harness.
...
### 1. 🏗️ [Architecture & Roadmap](./marmot_architecture_and_roadmap.md)
**Start Here.** This document covers the core philosophy of the project.
- The Microkernel Core Engine.
- The Decoupled Client-Server API (`marmot-protocol`).
...
The 3-tier memory architecture that enables Marmot to learn from mistakes.
- **Short-Term**: Context compaction and token budget management.
- **Episodic**: SQLite-backed session persistence (`marmot.db`).
...
The operational backbone of Marmot.
- `marmot update` auto-updater mechanics.
- `marmot-sim-llm` for deterministic, offline End-to-End testing.
...
How Marmot acts as a headless server to power native IDE extensions via WebSockets, stdio JSON-RPC, and MCP.
...
- Full, compilable Rust code for `marmot-protocol`, `marmot-tools`, `marmot-core`, `marmot-server`, and `marmot-tui`.
