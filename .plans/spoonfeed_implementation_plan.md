# Marmot: Foolproof "Spoon-Fed" Implementation Playbook
...
2. [Step 1: Crate `marmot-protocol` (Shared Types & Event Protocol)](#step-1-crate-marmot-protocol)
3. [Step 2: Crate `marmot-tools` (Primitive Filesystem & Shell Tools)](#step-2-crate-marmot-tools)
4. [Step 3: Crate `marmot-core` (Master Loop, Event Bus & Mock Provider)](#step-3-crate-marmot-core)
5. [Step 4: Crate `marmot-server` (Headless Event-Driven Server)](#step-4-crate-marmot-server)
6. [Step 5: Crate `marmot-tui` (Ratatui Non-Blocking Terminal Frontend)](#step-5-crate-marmot-tui)
7. [Step 6: Crate `marmot-cli` (Unified Executable Entrypoint)](#step-6-crate-marmot-cli)
...
resolver = "2"
members = [
    "crates/marmot-protocol",
    "crates/marmot-tools",
    "crates/marmot-core",
    "crates/marmot-server",
    "crates/marmot-tui",
    "crates/marmot-cli"
]
...
### 1.1 Create `crates/marmot-protocol/Cargo.toml`
- **File**: `/home/hunk/work/playground/crates/marmot-protocol/Cargo.toml`
- **Content**:
```toml
[package]
...
### 1.2 Create `crates/marmot-protocol/src/lib.rs`
- **File**: `/home/hunk/work/playground/crates/marmot-protocol/src/lib.rs`
- **Content**:
```rust
...
/// Commands sent from any UI/Client to the Marmot Core Engine
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Events broadcast by the Marmot Core Engine to all connected UIs/Clients
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(tag = "type", content = "payload")]
...
mod tests {
    ...
    fn test_serialization_roundtrip() {
        let cmd = MarmotCommand::SubmitPrompt {
...
### 2.1 Create `crates/marmot-tools/Cargo.toml`
- **File**: `/home/hunk/work/playground/crates/marmot-tools/Cargo.toml`
- **Content**:
```toml
[package]
name = "marmot-tools"
...
[dependencies]
marmot-protocol = { path = "../marmot-protocol" }
...
### 2.2 Create `crates/marmot-tools/src/lib.rs`
- **File**: `/home/hunk/work/playground/crates/marmot-tools/src/lib.rs`
- **Content**:
```rust
use async_trait::async_trait;
use marmot_protocol::{RiskLevel, ToolResult};
...
### 3.1 Create `crates/marmot-core/Cargo.toml`
- **File**: `/home/hunk/work/playground/crates/marmot-core/Cargo.toml`
- **Content**:
```toml
[package]
name = "marmot-core"
...
[dependencies]
marmot-protocol = { path = "../marmot-protocol" }
marmot-tools = { path = "../marmot-tools" }
tokio = { workspace = true }
async-trait = { workspace = true }
serde = { workspace = true }
serde_json = { workspace = true }
...
### 3.2 Create `crates/marmot-core/src/provider.rs`
- **File**: `/home/hunk/work/playground/crates/marmot-core/src/provider.rs`
- **Content**:
```rust
use async_trait::async_trait;
use marmot_protocol::ToolCall;
use tokio::sync::mpsc;
...
### 3.3 Create `crates/marmot-core/src/permissions.rs`
- **File**: `/home/hunk/work/playground/crates/marmot-core/src/permissions.rs`
- **Content**:
```rust
use marmot_protocol::{PermissionDecision, PermissionMode, RiskLevel};
...
### 3.4 Create `crates/marmot-core/src/engine.rs`
- **File**: `/home/hunk/work/playground/crates/marmot-core/src/engine.rs`
- **Content**:
```rust
use crate::permissions::PermissionBroker;
use crate::provider::{LlmProvider, ProviderStep};
use marmot_protocol::{
    AgentMode, MarmotCommand, MarmotEvent, PermissionDecision, PermissionMode,
};
use marmot_tools::ToolRegistry;
use std::sync::Arc;
...
pub struct MarmotEngine {
    ...
    tools: Arc<dyn marmot_protocol::ToolProvider>, // Strict Dependency Inversion
...
impl MarmotEngine {
    pub fn new(
        provider: Arc<dyn LlmProvider>, 
        tools: Arc<dyn marmot_protocol::ToolProvider>, 
        mode: PermissionMode
    ) -> Self {
        Self {
            provider,
    ...
    pub async fn run_turn(
        ...
        prompt: &str,
        mode: AgentMode,
        event_tx: mpsc::Sender<MarmotEvent>,
        mut perm_rx: mpsc::Receiver<(String, PermissionDecision)>,
    ) {
        let _ = event_tx.send(MarmotEvent::TurnStarted { turn_id: turn_id.to_string(), mode }).await;
        ...
        while let Some(step) = step_rx.recv().await {
            match step {
                ProviderStep::Token(chunk) => {
                    let _ = event_tx.send(MarmotEvent::TokenDelta { chunk }).await;
                }
                ProviderStep::CallTool(call) => {
                    let _ = event_tx.send(MarmotEvent::ToolCallInitiated(call.clone())).await;
                    let tool = match self.tools.get(&call.tool_name) {
                        Some(t) => t,
                        None => {
                            let _ = event_tx.send(MarmotEvent::ToolCallCompleted(marmot_protocol::ToolResult {
                    ...
                    let authorized = match self.permissions.check(&call.tool_name, risk) {
                        ...
                        None => {
                            ...
                            let _ = event_tx.send(MarmotEvent::PermissionRequested {
                    ...
                    if authorized {
                        ...
                        let _ = event_tx.send(MarmotEvent::ToolCallCompleted(result)).await;
                    } else {
                        let _ = event_tx.send(MarmotEvent::ToolCallCompleted(marmot_protocol::ToolResult {
                            call_id: call.call_id,
                            success: false,
        ...
        let _ = event_tx.send(MarmotEvent::TurnCompleted { turn_id: turn_id.to_string(), success: true }).await;
    }
}
```

### 3.5 Create `crates/marmot-core/src/lib.rs`
- **File**: `/home/hunk/work/playground/crates/marmot-core/src/lib.rs`
- **Content**:
```rust
pub mod engine;
pub mod permissions;
...
mod tests {
    use super::*;
    use marmot_protocol::{AgentMode, MarmotEvent, PermissionDecision, PermissionMode, ToolCall};
    use std::sync::Arc;
    use tokio::sync::mpsc;
    ...
    async fn test_master_loop_with_mock_provider() {
        ...
        let engine = MarmotEngine::new(mock_provider, PermissionMode::Open);
        ...
        while let Some(evt) = event_rx.recv().await {
            match evt {
                MarmotEvent::TokenDelta { chunk } => {
                    if chunk == "Hello" { received_token = true; }
                }
                MarmotEvent::TurnCompleted { .. } => {
                    completed = true;
                    break;
                }
                _ => {}
...
### 4.1 Create `crates/marmot-server/Cargo.toml`
- **File**: `/home/hunk/work/playground/crates/marmot-server/Cargo.toml`
- **Content**:
```toml
[package]
name = "marmot-server"
...
[dependencies]
marmot-protocol = { path = "../marmot-protocol" }
marmot-core = { path = "../marmot-core" }
tokio = { workspace = true }
async-trait = { workspace = true }
tracing = { workspace = true }
```

### 4.2 Create `crates/marmot-server/src/lib.rs`
- **File**: `/home/hunk/work/playground/crates/marmot-server/src/lib.rs`
- **Content**:
```rust
use marmot_core::MarmotEngine;
use marmot_protocol::{MarmotCommand, MarmotEvent, PermissionDecision};
use std::sync::Arc;
use tokio::sync::mpsc;

pub struct MarmotSession {
    engine: Arc<MarmotEngine>,
    cmd_rx: mpsc::Receiver<MarmotCommand>,
    event_tx: mpsc::Sender<MarmotEvent>,
...
impl MarmotSession {
    ...
    pub async fn run_loop(mut self) {
        ...
        while let Some(cmd) = self.cmd_rx.recv().await {
            match cmd {
                MarmotCommand::SubmitPrompt { prompt, mode } => {
                ...
                MarmotCommand::ResolvePermission { request_id, decision } => {
                    let _ = self.perm_tx.send((request_id, decision)).await;
                }
                MarmotCommand::AbortTurn => {
                    let _ = self.event_tx.send(MarmotEvent::Error { message: "Turn aborted by user".to_string() }).await;
                }
                _ => {}
            }
        }
...
### 5.1 Create `crates/marmot-tui/Cargo.toml`
- **File**: `/home/hunk/work/playground/crates/marmot-tui/Cargo.toml`
- **Content**:
```toml
[package]
name = "marmot-tui"
...
### 5.2 Create `crates/marmot-tui/src/lib.rs`
- **File**: `/home/hunk/work/playground/crates/marmot-tui/src/lib.rs`
- **Content**:
```rust
use crossterm::{
...
use marmot_protocol::{AgentMode, MarmotCommand, MarmotEvent, PermissionDecision};
use ratatui::{
...
impl TuiApp {
    ...
    pub async fn run(
        mut self,
        cmd_tx: mpsc::Sender<MarmotCommand>,
        mut event_rx: mpsc::Receiver<MarmotEvent>,
        ...
        loop {
            terminal.draw(|f| {
                ...
                // 1. Header
                let header = Paragraph::new(" Marmot v0.1.0 │ Mode: Build │ Security: Review-for-me")
                ...
                Some(Ok(event)) = reader.next() => {
                    if let Event::Key(key) = event {
                        ...
                        if let Some((req_id, _)) = self.pending_permission.take() {
                            match key.code {
                                KeyCode::Char('y') => {
                                    let _ = cmd_tx.send(MarmotCommand::ResolvePermission {
                                        request_id: req_id,
                        ...
                        match key.code {
                            ...
                            KeyCode::Enter => {
                                if !self.input.trim().is_empty() {
                                    ...
                                    let _ = cmd_tx.send(MarmotCommand::SubmitPrompt {
                                        prompt,
                                        mode: AgentMode::Build,
                                    }).await;
                                }
...
```bash
cargo check -p marmot-tui
```

---

## Step 6: Crate `marmot-cli`

The unified binary combining Engine, Mock/Anthropic Provider, Server, and TUI.

### 6.1 Create `crates/marmot-cli/Cargo.toml`
- **File**: `/home/hunk/work/playground/crates/marmot-cli/Cargo.toml`
- **Content**:
```toml
[package]
name = "marmot-cli"
...
marmot-protocol = { path = "../marmot-protocol" }
marmot-core = { path = "../marmot-core" }
marmot-server = { path = "../marmot-server" }
marmot-tui = { path = "../marmot-tui" }
tokio = { workspace = true }
clap = { version = "4.5", features = ["derive"] }
```

### 6.2 Create `crates/marmot-cli/src/main.rs`
- **File**: `/home/hunk/work/playground/crates/marmot-cli/src/main.rs`
- **Content**:
```rust
use clap::Parser;
use marmot_core::{LlmProvider, MarmotEngine, MockProvider, ProviderStep};
use marmot_protocol::{PermissionMode, ToolCall};
use marmot_server::MarmotSession;
use marmot_tui::TuiApp;
use std::sync::Arc;
use tokio::sync::mpsc;

#[derive(Parser, Debug)]
#[command(name = "marmot", about = "Fast, modular agentic coding harness")]
struct Args {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mock_provider = Arc::new(MockProvider {
        responses: vec![
            ProviderStep::Token("Welcome to Marmot. Analyzing your repository... ".to_string()),
            ProviderStep::CallTool(ToolCall {
    ...
    let engine = Arc::new(MarmotEngine::new(mock_provider, PermissionMode::ReviewForMe));

    let (cmd_tx, cmd_rx) = mpsc::channel(32);
    let (event_tx, event_rx) = mpsc::channel(64);

    let session = MarmotSession::new(engine, cmd_rx, event_tx);
    tokio::spawn(async move {
...
```
*(Expected: Full workspace builds cleanly into `target/debug/marmot-cli`).*
...
2. `crates/marmot-protocol/Cargo.toml` & `src/lib.rs`
3. `crates/marmot-tools/Cargo.toml` & `src/lib.rs`
4. `crates/marmot-core/Cargo.toml`, `src/lib.rs`, `src/provider.rs`, `src/permissions.rs`, `src/engine.rs`
5. `crates/marmot-server/Cargo.toml` & `src/lib.rs`
6. `crates/marmot-tui/Cargo.toml` & `src/lib.rs`
7. `crates/marmot-cli/Cargo.toml` & `src/main.rs`
