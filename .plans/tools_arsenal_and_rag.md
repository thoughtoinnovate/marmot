# Marmot: The Tools Arsenal & Code Exploration Strategy
...
Marmot adopts a **Deterministic-First** approach: AST (Abstract Syntax Tree) and Lexical search are vastly superior to probabilistic RAG for writing code.

---

## 1. The Core Arsenal (Lexical Primitives)

These tools are built directly into `marmot-tools` and are used for 90% of daily tasks. They are blisteringly fast.
...
Why guess with Vector math when the compiler knows exactly where the code is? Marmot leverages its `marmot-lsp` bridge as active exploration tools.
...
*Best Practice*: When Marmot needs to understand how `PermissionBroker` works, it shouldn't "search the web" or "query RAG". It should call `goto_definition("PermissionBroker")`, instantly landing on the exact struct definition.
...
Because **every tool is a plugin**, the internal `marmot-plugin-interaction` registers the following tools to the event bus:
...
### The Marmot Solution: `marmot-plugin-rag`
To keep the core system extremely lightweight (running on old machines), Marmot does **not** force a Vector DB on you. 

Instead, it is handled via the plugin system:
* If you are working in a giant enterprise codebase (10,000+ files) where you need to ask vague conceptual questions (*"Where is the logic that calculates tax for European customers?"*), you can load the `marmot-plugin-rag`.
* This plugin uses `sqlite-vec` (a lightweight vector extension for our existing SQLite `marmot.db`) and a fast local embedding model (like `all-MiniLM-L6-v2` via ONNX).
* It provides the `semantic_search` tool to the agent.

**Summary**: Marmot relies on **Ripgrep + LSP** for absolute precision, and offers **Local SQLite RAG** as an optional plugin for massive conceptual exploration.
